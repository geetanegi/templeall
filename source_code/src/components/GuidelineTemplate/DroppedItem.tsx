...
import { updateElement } from '../../redux/slice/template/templateSlice';
import { useDispatch, useSelector } from 'react-redux';
import { useLocation } from 'react-router-dom';
import JoditReact from 'jodit-react-ts';
import 'jodit/build/jodit.min.css';
import ClientProviderDropdown from './ClientProviderDropdown';
...
export default function DroppedItem({
    ...
    React.useEffect(() => {
        if (isSessionNote) {
            ...
            const filteredVal = template?.filter((item: any) => {
                return item.name === 'editor';
            });
            const placeholderElement = document.querySelector(
                'span[data-ref="placeholder"]'
            );
            if (placeholderElement && filteredVal) {
                placeholderElement.remove();
            }
            if (textarea && filteredVal?.length) {
                textarea.innerHTML = filteredVal[0]?.value;
            }
        }
    }, []);
    ...
    // const moveCursorToEnd = (contentEle: any): void => {
    //     const sel = window.getSelection();
    //     if (sel) {
    //         sel.selectAllChildren(contentEle);
    //         sel.collapseToEnd();
    //     }
    // };
        ...
        >
            ...
            <div className="flex-col justify-baseline">
                ...
                <div
                    ...
                    ${isSessionNote ? ' border-[#ABABAB] p-4 rounded-md' : 'border-gray-500 p-4'} 
                    w-full`}
                >
                    {renderHeading()}
