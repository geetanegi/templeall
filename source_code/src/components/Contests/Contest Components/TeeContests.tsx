...
const TeeContests: React.FC<{ teeContest: TeeContest }> = ({ teeContest }) => {
  ...
  //   ? moment(teeContest.eligibleRegistrationTime)
  //   : null;
  ...
  // const registrationStart = moment(teeContest.registrationStartTime);
  //  isUtcDate(teeContest.registrationStartTime)
  //   ? moment(teeContest.registrationStartTime) // already in UTC
  //   : moment(teeContest.registrationStartTime); // treat as UTC

  // const registrationEnd =moment(teeContest.registrationEndTime) 
  //  isUtcDate(teeContest.registrationEndTime)
        ...
        <div className="rounded-[6px] border shadow-md">
          <div className="my-2 flex w-full items-center justify-between px-6 pt-6">
            ...
            <div className="flex flex-col place-items-end">
              <p className="text-xs font-normal text-[#3C3C4399]">{`${moment.utc(teeContest.registrationStartTime).local().format("MM/DD/YYYY hh:mm A")} - ${moment.utc(teeContest.registrationEndTime).local().format("MM/DD/YYYY hh:mm A")} `}</p>{" "}
            ...
            <div>
              ...
              */}
              ...
              //     teeContest?.eligibleForRegistration) &&
              //   isCrossed 
                teeContest.checkValidRegistration
                && (
                  <>
                    {isSelected ? (
                      <Minus
          ...
          <div className="px-2 text-right text-xs text-red-500">
            ...
            <span className="text-xs">{!teeContest.checkValidRegistration && teeContest.errorMessage}</span>
          </div>
