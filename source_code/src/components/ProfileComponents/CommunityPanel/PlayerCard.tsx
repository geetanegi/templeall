...
const PlayerCard: React.FC<PlayerCardProps> = ({ profile, isSelected, setSelectedUser }) => {
    ...
    return (
        <div className=' w-[242px] my-5' >
            <div className={`flex  cursor-pointer py-2 pl-2   
                 ${isSelected? "h-[64px] w-[212px] bg-[#1D1A0CB2] rounded-t-md ml-auto" : "h-[64px] w-[242px] bg-[#FFFFFFB2] rounded-md"}
            `}
            ...
            >
                ...
                <div className='flex justify-center flex-col ml-3 h-full text-gray-500 text-sm'>
                    ...
                    <span className={`text-[14px] tracking-[0.25px] ${isSelected ? "text-[#F5F6F7]" : "text-[#7B7887]"}`}>Detroit, MI</span>
                </div>
            </div>
