...
interface AddProgramBookModalProps {
    readonly open: boolean;
    readonly onClose: () => void;
    readonly dataProgramBook: any;
}

const ProgramBookHistory: React.FC<AddProgramBookModalProps> = ({
    dataProgramBook,
    open,
    onClose,
}) => {
    ...
    const columnDefinitionsTemplateGrid = [
        ...
        },
        {
            ...
            width: '',
            body: (e: any) =>
                CustomName(e?.executedBy?.firstName, e?.executedBy?.lastName),
        },
        {
            header: ConstColumnDiv(
                ...
                getGridData,
                '',
                dataProgramBook?.programBookUUID
            ),
            width: '',
    ...
    return (
        <Modal open={open} id={'add-Client-Doc-modal'} expandModal={false}>
            <ModalHeader
                title={`${dataProgramBook?.name} - Program Book History`}
                icon={false}
                onExpand={undefined}
                onClose={onClose}
                closeIcon={true}
            ...
            <div className="flex flex-col p-5 min-w-[80rem] min-h-[40rem] overflow-auto">
                ...
                <Grid
                    id={dataProgramBook?.programBookUUID}
...
export default ProgramBookHistory;
