...
const TooltipSpan: React.FC<TooltipSpan> = ({
  ...
  switch (position) {
    case "top":
      tooltipStyles = "bottom-full left-1/2 transform -translate-x-1/2 ";
      arrowStyles =
  ...
  return (
    ...
    >
      ...
      {isVisible && (
        <div
          role="tooltip"
          className={`absolute z-10 w-[180px] inline-block text-center rounded-lg bg-gray-900 px-3 py-2 text-[13px] font-medium text-white opacity-100 shadow-sm transition-opacity duration-300 ${tooltipStyles}`}
        >
          {tooltip}
          {/* Tooltip arrow */}
